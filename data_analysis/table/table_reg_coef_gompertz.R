...
# normal gompertz
gom_norm <- read.csv(file=paste0(dir_results, "parameters_normal_gompertz.csv"))
names(gom_norm) <- c("para", "norm", "norm.lower", "norm.upper")

# clayton gompertz
gom_cly <- read.csv(file=paste0(dir_results, "parameters_clayton_gompertz.csv"))
names(gom_cly) <- c("para", "clayton", "clayton.lower", "clayton.upper")

# frank gompertz
gom_frank <- read.csv(file=paste0(dir_results, "parameters_frank_gompertz.csv"))
names(gom_frank) <- c("para", "frank", "frank.lower", "frank.upper")

# gumbel gompertz
gom_gum <- read.csv(file=paste0(dir_results, "parameters_gumbel_gompertz.csv"))
names(gom_gum) <- c("para", "gum", "gum.lower", "gum.upper")

df <- merge(gom_norm, gom_cly, by.x = "para")

df <- merge(df, gom_frank, by.x = "para")

df <- merge(df, gom_gum, by.x = "para")

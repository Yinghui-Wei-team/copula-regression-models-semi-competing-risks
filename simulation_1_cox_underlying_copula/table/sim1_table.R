...
df <- cbind(cox_normal, cox_gumbel, cox_frank, cox_gumbel)
...
index <- which(model1_gumbel == "b0")
model1_gumbel <- model1_gumbel[-index,]
names(model1_gumbel) <- c("items", "gumbel_bias", "gumbel_cp", "gumbel_mse")
...
df <- cbind(model1_normal, model1_gumbel, model1_frank, model1_gumbel)
...
df <- df %>% mutate(gumbel_cp = format(round(100*(gumbel_cp/100),1), nsmall=1)) %>%

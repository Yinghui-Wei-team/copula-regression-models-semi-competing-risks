...
# Model1: Frank copula survival model with covariates for hazard rates
...
# 1. reset results directory, specify out files, put n and runs nearer to the front of the script
##################################################################################################
...
start_a0 <- -6; start_a1 <- -4.5; start_a2 <- -4.5; start_a3 <- -4.5;
start_c0 <- -6; start_c1 <- -3.5; start_c2 <- -4.5; start_c3 <- -4.5;
start_b0 <- 5;
starting_values <- c(start_a0, start_a1, start_a2, start_a3, start_c0, start_c1, start_c2, start_c3, start_b0)

# True values of parameters (also defined later):
...
for (i in 1:runs){
  ...
  #a ci
  save_a0[i] <- a0_est <- plfoptim$par[1]
  save_a1[i] <- a1_est <- plfoptim$par[2]
  save_a2[i] <- a2_est <- plfoptim$par[3]
  save_a3[i] <- a3_est <- plfoptim$par[4]
  ...
  #c ci
  save_c0[i] <- c0_est <- plfoptim$par[5]
  save_c1[i] <- c1_est <- plfoptim$par[6]
  save_c2[i] <- c2_est <- plfoptim$par[7]
  save_c3[i] <- c3_est <- plfoptim$par[8]
  ...
  save_hr_l1_age[i] <- esthr_l1_age <- exp(a1_est)
  save_hr_l1_gen[i] <- esthr_l1_gen <- exp(a2_est)
  save_hr_l1_donor[i] <- esthr_l1_donor <- exp(a3_est)
  
  save_hr_l2_age[i] <- esthr_l2_age <- exp(c1_est)
  save_hr_l2_gen[i] <- esthr_l2_gen <- exp(c2_est)
  save_hr_l2_donor[i] <- esthr_l2_donor <- exp(c3_est)
...
print("Simulation1 model1 for frank exponential model completed successfully!")
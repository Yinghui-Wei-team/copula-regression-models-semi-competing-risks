...
# YW, 27 July 2021: 1. correct bias, mse and re-calculate mse without using loop
...
out_file_summary <- "S2-misspecification - underlying gumbel weibull - summary.csv"
out_file_estimates <-"S2-misspecification - underlying gumbel weibull - estimates.csv"


#####################################################################################
################## Gumbel, age.grp, gen from wei chose with aic #######################
#####################################################################################
...
true_a1 <- 0.68 #weib alpha1
true_a2 <- 0.97 #weib alpha2
true_x0 <- -2.72 #weib beta1
true_x1 <- 0.05 #weib beta1
true_y0 <- -4.23 #weib beta2
true_y1 <- 1.35 #weib beta2
...
for (i in 1:runs){
  ...
  #Step 1: generate age.grp categories
  age.grp <- rbinom(n,1,0.40)          #40% are in the older age.grp group in NHSBT data
  
  for(k in 1: n){   #loop to generate U an V from age.grp-varying theta
    m=1                  
    ...
    true_beta1[k] <- exp(true_x0 + true_x1 * age.grp[k])
    true_beta2[k] <- exp(true_y0 + true_y1 * age.grp[k]) 
  ...
  T1 <- (-log(U1)/true_beta1)^(1/true_a1)
  T2 <- (-log(V1)/true_beta2)^(1/true_a2)
  ...
  gumbel_exp_optim_starting_values = c(-3, 0, -3, 0, -1, 1) # starting values 
  #gumbel_exp_optim_starting_values = c(-3, 0, -3, 0, true_b0, true_b1) # starting values 
  ...
  # b1_up <- 3
  # 
  # plgoptim_exp <- optim(c(-3, 0, -3, 0, true_b0, true_b1), gpl_exp, method="L-BFGS-B",
  ...
  # starting values and boundaries for a1, x0, x1, a2, y0, y1, b0, b1
  gumbel_wei_optim_lower = c(0.01, -10.00, -10.00,   0.01, -10.00, -10.00, -10.00, -15.00) # lower bound 
  gumbel_wei_optim_upper = c(1.5, -1.0,  1.0,  1.5, -2.0,  2.0,  1.0,  2.0)# upper bound 
  gumbel_wei_optim_starting_values =c(0.6, -2, 0, 0.9, -4, 1.5, -1.5, 1) # starting values, changed by YW
  ...
  # b1_up <- 2
  # 
  # plgoptim_wei <- optim(c(true_a1, true_x0, true_x1, true_a2, true_y0, true_y1, true_b0, true_b1), gpl_wei, method="L-BFGS-B",
  ...
  # likelihood function moved out of loop by YW
  # YW rewrote
  gumbel_gom_optim_lower =           c(-0.2, -5.0, -4.0, -0.2, -6.0, -2.0, -4.0, -2.0) # lower bound
  gumbel_gom_optim_upper =           c(0.1, -2.0,   2.0,  0.1, -2.0,  3.0,  1.2,  2.3)# upper bound 
  gumbel_gom_optim_starting_values = c(0.01,  -3,   0.1,  0.02, -4,    1,   -1.5, 1) # starting values, changed by YW 
  ...
  # b1_up <- 2.3
  # 
  # plgoptim_gom <- optim(c(0.01, -3, 0.1,0.02, -4, 1, true_b0, true_b1), gpl_gom, method="L-BFGS-B",
  ...
  # YW: checking the following code works for index =1, 2, 3
  if(i==1) index =1
  if(i==2) index =3

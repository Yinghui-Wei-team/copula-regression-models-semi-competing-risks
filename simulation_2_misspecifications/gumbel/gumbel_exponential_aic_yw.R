...
#                   6. grammar error "break" should be replaced by "next"
...
out_file_summary <- "S2-misspecification - underlying gumbel exponential - summary.csv"
out_file_estimates <-"S2-misspecification - underlying gumbel exponential - estimates.csv"

#####################################################################################
################### Gumbel, age.grp, gen from exp chose with aic ####################
#####################################################################################
...
true_a0 <- -3.46 #exp lambda1
true_a1 <- 0.18 #exp lambda1
true_c0 <- -4.36 #exp lambda2
true_c1 <- 1.36 #exp lambda2
...
# YW: added constraint for theta >=1 for gumbel copula

counter_theta_out_of_bound = 0
...
counter_hr_l1 = counter_hr_l2 = counter_rho_d0 = counter_rho_d1 = 0
# commented out by YW 
# ## stuff for later ##
...
#-------YW: likelihood function specification ---------------------------#
########################################################
...
for (i in 1:runs){
  ...
  #Step 1: generate gen categories
  age.grp <- rbinom(n,1,0.40)          #40% are in the older gen group in NHSBT data
  
  for(k in 1:n){   #loop to generate U an V from gen-varying theta
    m=1                  
    ...
    true_l1[k] <- exp(true_a0 + true_a1 * age.grp[k])
    true_l2[k] <- exp(true_c0 + true_c1 * age.grp[k]) 
  ...
  # lower bound for b0 must be 0, because theta must be >=1 for gumbel
  #rewritten by YW
  gumbel_exp_optim_lower = c(-10, -10, -10, -10,  -5,  -5) # lower bound 
  gumbel_exp_optim_upper = c(-2.0,  1.5, -2.0,  3.0, -1.0,  4.0)# upper bound 
  # YW changed the starting values to the middle values of lower and upper instead of the true values
  gumbel_exp_optim_starting_values = c(-6.00, -4.25, -6.00, -3.50, -3.00, -0.50) # starting values 
   #gumbel_exp_optim_starting_values = c(true_a0, true_a1, true_c0, true_c1, true_b0, true_b1)
  ...
  # b1_up <- 4
  # 
  # plgoptim_exp <- optim(c(true_a0, true_a1, true_c0, true_c1, true_b0, true_b1), gpl_exp, method="L-BFGS-B",
  ...
  # rewritten by YW
  # Isn't the lower bound for b0 and b1 must be 0, because theta must be >=1 for gumbel
  
  gumbel_wei_optim_lower = c(0.01, -10.00, -10.00,   0.01, -10.00, -10.00,  -6,  -5) # lower bound 
  gumbel_wei_optim_upper = c(1.5, -1.5,  1.0,  1.5, -2.0,  2.5, -1.0,  3.0)# upper bound 
  gumbel_wei_optim_starting_values =c(0.6, true_a0, true_a1, 0.6, true_c0, true_c1, true_b0, true_b1) # starting values 
  ...
  # b1_up <- 3
  # 
  # plgoptim_wei <- optim(c(0.6, true_a0, true_a1, 0.6, true_c0, true_c1, true_b0, true_b1), gpl_wei, method="L-BFGS-B",
  ...
  # if(plgoptim_wei$par[2] == x0_lw) {break}
  # if(plgoptim_wei$par[2] == x0_up) {break}
  # if(plgoptim_wei$par[3] == x1_lw) {break}
  # if(plgoptim_wei$par[3] == x1_up) {break}
  # if(plgoptim_wei$par[4] == a2_lw) {break}
  # if(plgoptim_wei$par[4] == a2_up) {break}
  # if(plgoptim_wei$par[5] == y0_lw) {break}
  # if(plgoptim_wei$par[5] == y0_up) {break}
  # if(plgoptim_wei$par[6] == y1_lw) {break}
  # if(plgoptim_wei$par[6] == y1_up) {break}
  ...
  # likelihood function moved out of loop by YW 
  # lower bound for b0 and b1 must be 0, because theta must be >=1 for gumbel
  gumbel_gom_optim_lower = c(-0.2, -5.0, -4.0, -0.2, -6.0, -4.0, -4,  0.0) # lower bound
  gumbel_gom_optim_upper = c(0.1, -2.0,  2.0,  0.1, -2.0,  3.0, -1.0,  3.0)# upper bound 
  gumbel_gom_optim_starting_values =gumbel_gom_optim_lower # starting values 
  # checking lower == gumbel_gom_optim_lower
  ...
  # b1_up <- 3
  # 
  # plgoptim_gom <- optim(c(g1_lw,p0_lw,p1_lw,g2_lw,q0_lw, q1_lw, b0_lw,b1_lw), gpl_gom, method="L-BFGS-B",
  ...
  if (index==1){ #chooses exponential
    ...
    if(true_hr_l1 <= hr_l1_upci[i] && true_hr_l1 >= hr_l1_lwci[i]) {counter_hr_l1=counter_hr_l1+1}
    if(true_hr_l2 <= hr_l2_upci[i] && true_hr_l2 >= hr_l2_lwci[i]) {counter_hr_l2=counter_hr_l2+1}
    
    if(true_rho_d0 <= rho_d0_upci[i] && true_rho_d0 >= rho_d0_lwci[i]) {counter_rho_d0=counter_rho_d0+1}
    if(true_rho_d1 <= rho_d1_upci[i] && true_rho_d1 >= rho_d1_lwci[i]) {counter_rho_d1=counter_rho_d1+1}
...
print(counter_theta_out_of_bound)
# output results

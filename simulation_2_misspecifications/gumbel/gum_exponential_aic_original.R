# content: model selection by AIC, underlying model: gumbel copula exponential survival model
# YW minor changes: keep code as original but added timer and set working directory and outfile
rm(list=ls())
library(copula)
library(mvtnorm)
library(plyr)
...
#####################################################################################
##################Timer, working directory and outfile        #######################
#####################################################################################
...
outfile_estimates <- "aic_a_gumbel_exp_1.csv" # a-covariate age, exp-exponential
...
for (i in 1:runs){
  ...
  if (index==1){ #chooses exponential
    ...
    # YW: checking
    #print(c(theta_d0, theta_d0_lwci, theta_d0_upci))
    ...
    #YW: checking
    #print(c(rho_d0_lwci_cop, rho_d0_upci_cop))
    #rho for gen=1
...
write.csv(df2, file=outfile_estimates)

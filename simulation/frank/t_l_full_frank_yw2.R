...
# Modle: Frank copula exponential survival model 
# with covariates on hazard rates and association parameter
###########################################################
# YW 27 June 2021 updates: 
...
for (i in 1:runs){
  ...
  #fpl(c(-0.5,0,-0.5,0,5,2),X=df$X, Y=df$Y, d1=df$d1, d2=df$d2, age.grp=df$age.grp)
  ...
  # plfoptim <- optim(c(true_a0, true_a1, true_a2, true_a3, true_c0, true_c1, true_c2, true_c3,
  #                      true_b0, true_b1, true_b2, true_b3), fpl, method="L-BFGS-B", 
  ...
  plfoptim <- optim(c(-4, 0.5, 0.5,-1, -5, -2, -1, -1,
                     4, 6, 1, 1), fpl, method="L-BFGS-B", 
                    lower=c(a0_lw,a1_lw,a2_lw, a3_lw,c0_lw,c1_lw,c2_lw, c3_lw, b0_lw,b1_lw, b2_lw, b3_lw),
                    upper=c(a0_up,a1_up,a2_up, a3_up, c0_up,c1_up,c2_up,c3_up, b0_up,b1_up,b2_up, b3_up), 
                    X=df$X, Y=df$Y, d1=df$d1, d2=df$d2, age.grp=df$age.grp, gen=df$gen,
                    donor=df$donor, control=list(fnscale=-1),hessian=TRUE)
 ...
 fpl(c(a0_up,a1_up,a2_up, a3_up, c0_up,c1_up,c2_up,c3_up, b0_up,b1_up,b2_up, b3_up), X=df$X, Y=df$Y, d1=df$d1, d2=df$d2, age.grp=df$age.grp, gen=df$gen, donor=df$donor)
  ...
  if(plfoptim$par[9] == b0_lw) {counter_b0_low = counter_b0_low + 1}
  if(plfoptim$par[9] == b0_up) {counter_b0_upper = counter_b0_upper + 1}
  if(plfoptim$par[10] == b1_lw) {counter_b1_low = counter_b1_low + 1}
  if(plfoptim$par[10] == b1_up) {counter_b1_upper = counter_b1_upper + 1}
  if(plfoptim$par[11] == b2_lw) {counter_b2_low = counter_b2_low + 1}
  if(plfoptim$par[11] == b2_up) {counter_b2_upper = counter_b2_upper + 1}
  if(plfoptim$par[12] == b3_lw) {counter_b3_low = counter_b3_low + 1}
  if(plfoptim$par[12] == b3_up) {counter_b3_upper = counter_b3_upper + 1}
  ...
  b1_est <- plfoptim$par[10]
  b2_est <- plfoptim$par[11]
  b3_est <- plfoptim$par[12]
  save_b0[i] <- b0_est
  save_b1[i] <- b1_est
  save_b2[i] <- b2_est
  save_b3[i] <- b3_est
...
#variance: corrected YW 27 June 2021
b0_var <- var(save_b0)
b1_var <- var(save_b1)
b2_var <- var(save_b2)
b3_var <- var(save_b3)
...
#variance: corrected by YW 27 June 2021
a0_var <- var(save_a0)
a1_var <- var(save_a1)
a2_var <- var(save_a2)
a3_var <- var(save_a3)
...
# YW 27 June 2021: put results together and write to CSV file
# mean of bias
...
write.csv(Results, row.names=F,file="S2-Table5-frank-exponential-covariates-hazards-association.csv")
...
write.csv(Estimates, row.names=F,file="S2-Frank-exponential-covariates-for-hazards-estimates.csv")
